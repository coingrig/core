  ...
  getDecimals = async (): Promise<number | null> => {
    if (this.config.decimals === null) {
      ...
      if (!this.config.contract) {
        throw new Error('Wallet decimals not set, can not retrieve them without contract address!')
      }
      ...
      let driver = new this.BALANCE_DRIVER_NAMESPACE[driverDescription.driver](this.config, driverDescription.config);

      const provider = new Web3.providers.HttpProvider(driver.getTokenBalanceEndpoint());
